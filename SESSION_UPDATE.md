# Session Update — POC Data Engineering Platform
...
1. **Validated the project context.** Found a major gap between docs and disk: `CLAUDE.md` claimed a fully built scaffold, but only loose root-level files existed (chat artifact dump, no git, ~30 WSL2 metadata files, two unextracted ZIPs containing competing stack iterations).
...
8. **Authored docs**: `README.md` (concise), `docs/ONBOARDING.md` (team walkthrough), `docs/poc-overview.md` (canonical project brief from the original Evalueserve doc).
9. **Reconciled `CLAUDE.md`** to disk truth — Done/TODO blocks now reflect what genuinely exists.
...
- **Tree** matches the layout in [CLAUDE.md](CLAUDE.md) (Databricks-only)
- **Deps** installed in `.venv` via `uv sync` + `uv sync --group dev` (writes `uv.lock`)
- **Smoke tests** green (`uv run pytest`)
- **Lint** clean (`uv run ruff check .`)
- **Git**: initialized, first commit landed, `origin` remote added, **not pushed**
...
1. **Resolve `energy_ingestion_dag.py` env-awareness gap.** Currently hardcoded to `databricks-sql` mode + `kafka:9092` despite `CLAUDE.md` claiming env-aware switching. Make it read `ENV` from settings.
...
4. **Wire dbt with the `dbt-databricks` adapter.** Create `transform/dbt_project.yml`, `transform/profiles.yml`, then start on `transform/models/staging/{stg_scada,stg_weather,stg_demand,stg_dispatch,stg_plants}.sql`.
5. **Gold marts**: `fact_generation`, `fact_revenue`, `fact_dispatch`, `fact_carbon`, `dim_node`, `dim_plant`, `dim_time`.

### Sprint 4-5 work

6. **Databricks Asset Bundle** (`databricks.yml`) for DAB deploy.
7. **Flesh out CI/CD** workflows: source validation, offline DQ, Kafka integration test, DAB deploy with approval gate.
8. **Great Expectations** suites for Silver/Gold.
9. **Real test suite** (currently only smoke tests).

### After stakeholder review

10. **First push** to GitHub: `git push -u origin main` (do not push without confirming with the team).
...
- Does the team want to migrate from the custom `seed_producer.py + bronze_writer.py` to **Databricks Lakeflow Connect** (the screenshots suggest yes — "Insert in the Databricks streaming table using lakeflow connectors")? If so, the custom Kafka consumer becomes scaffolding only.
- Is the **Trello board** ("POC Evalueserve") the source of truth for sprint scope, or are we using GitHub Issues / Projects?
...
| Order | File | Why |
|---|---|---|
| 1 | [CLAUDE.md](CLAUDE.md) | Architecture, conventions, current Done/TODO |
